...
var laserdmg;
var laserStrt = 10000; //start after how many miliseconds
...
var playlvl1State = {
	...
	Ltween.onComplete.add(function(){
		laserAlrt.kill();
		var laser = game.add.sprite(rndX,650,'laser');
		laserEff.play();
	...
	playerhp: function(player,bullet) {
		...
		if(isinvul == false){
			...
			bullet.kill();
			game.time.events.add(invulnerable*1000, this.okBody, this);
	...
	playerhp2: function(player,hombullet){
		...
		//if(game.time.now - invulnerable >= 2000){ 
			...
			if (hp % 1 == 0){
				temporaryi.animations.play('fulltonone',5,false);
				temporaryi.animations.currentAnim.onComplete.add(function(){temporaryi.animations.play('none',1,true)},this);
			} else{
				temporaryi.animations.play('halftonone',5,false);
				temporaryi.animations.currentAnim.onComplete.add(function(){temporaryi.animations.play('none',1,true)},this);
				temporaryk.animations.play('fulltohalf',5,false);
	...
	damaged: function(){
		...
		//we compare hp and dmg for %1==0 or not
			case (preHP%1 == 0 && dmg%1 == 0): //2 -- 2
				...
				break;
			case (preHP%1 == 0 && dmg%1 != 0): //2 -- 2.5
				
			
				break;
			case (preHP%1 != 0 && dmg%1 == 0):

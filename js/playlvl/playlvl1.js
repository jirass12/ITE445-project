...
var playlvl1State = {
	...
	create: function(){
		...
		this.laser.createMultiple(10, 'laser');
		game.time.events.loop(10000, this.addlaser, this); //add 
		// laser 
		//game.add.sprite(0, 0, 'laser');
		//
		
		//alert 
	/* 	this.alert2 = game.add.group();
		game.physics.arcade.enable(this.alert2);
		this.alert2.createMultiple(10, 'alert2');
		game.time.events.loop(2000, this.addalert2, this); */
		//
	...
	update: function() {
		...
		game.physics.arcade.overlap(this.player, this.laser, this.playerhp3, null, this);
...
// laser destroy enemies
 		game.physics.arcade.overlap(this.laser, this.hombullet, this.laserdestroyhom, null, this);
		game.physics.arcade.overlap(this.laser, this.bullet, this.laserdestroybullet, null, this);
},
	levelreset: function(){
		lvl1timer = oritimer;
		hp = orihp;	
	...
	addlaser: function() {
		...
		laser.anchor.setTo(1);
		laser.reset(game.rnd.integerInRange(80,460) , 0);
	...
	//	alertBlink = game.add.tween(alert2).to( { alpha: 0.5 }, 300, null, true, 0, 3, true);
	...
	playerhp2: function(player,hombullet){
		...
		//if(game.time.now - invulnerable >= 2000){ 
			...
			if (hp % 1 == 0){
				temporaryi.animations.play('fulltonone',6,false);
				temporaryi.animations.currentAnim.onComplete.add(function(){temporaryi.animations.play('none',1,true)},this);
			} else{
				temporaryi.animations.play('halftonone',6,false);
				temporaryi.animations.currentAnim.onComplete.add(function(){temporaryi.animations.play('none',1,true)},this);
				temporaryk.animations.play('fulltohalf',6,false);
...
playerhp3: function(player,laser) {
			...
			playerblink = game.add.tween(player).to( { alpha: 0.5 }, 100, null, true, 0, 4, true);
			//Set a TimerEvent to occur after 2 seconds
		game.time.events.add(Phaser.Timer.SECOND * invulnerable/2, this.okBody, this);
		}	
},
	noBody: function(){
...
laserdestroyhom: function(laser,hombullet) {
	hombullet.destroy();

},
laserdestroybullet: function(laser,bullet) {
	bullet.destroy();
